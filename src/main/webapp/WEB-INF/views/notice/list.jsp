...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
  ...
  <!-- 
	...
	<div id="fh5co-page">
		...
		<section id="fh5co-hero" class="no-js-fullheight" style="background-image: url(/resources/images/bg3.jpg);" data-next="yes">
				...
				<div class="fh5co-intro no-js-fullheight">
					<div class="fh5co-intro-text">
						...
						<div class="fh5co-center-position">
							<h2 class="animate-box">공지사항</h2>
							<h3 class="animate-box">MCOS의 소식을 들려드립니다</h3>
	...
	<tbody>
		...
		<c:forEach items="${list}" var="list">
			<tr>
				<td>${list.noticeid}</td>
				<td>
					<a href="/notice/view?noticeid=${list.noticeid}">${list.title}</a>
				</td>
				<td>
		...
		<div class="container text-center">      
		  <table class="table table-hover" id="table-font">
		    <thead>
		      <tr>
		        <th>번호</th>
		        <th>제목</th>
...
<div class="container" style="text-align:center;font-size:25px;">
	...
	<%-- <c:forEach begin="1" end="${pageNum}" var="num">
		...
		<div class="navbar container form-group" style="float:right">
		<select name="searchType" class="fomr-control" style="font-size:24px; padding-bottom:0px;">
		    <option value="title" <c:if test="${page.searchType eq 'title'}">selected</c:if>>제목</option>
	        <option value="content" <c:if test="${page.searchType eq 'content'}">selected</c:if>>내용</option>
		    <option value="title_content" <c:if test="${page.searchType eq 'title_content'}">selected</c:if>>제목+내용</option>
		</select>
		
		<input type="text" class="fomr-control " name="keyword" value="${page.keyword}"/>
		
		<button type="button" class="btn btn-primary btn-sm" id="searchBtn">검색</button>
		<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" /> 
		<sec:authorize access="hasRole('ROLE_MEMBER,ROLE_ADMIN')">
		<span style="float:right"><a href="/notice/write" role="button" class="btn btn-info btn-sm">작성</a></span>
		</sec:authorize>
	</div>
